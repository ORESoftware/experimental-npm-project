...
-spec be_ok({ok, FII} | {error, any()}) -> FII.
be_ok(A) ->
    case A of
        {ok, Value} ->
            Value;
...
-spec be_error({ok, any()} | {error, FIN}) -> FIN.
be_error(A) ->
    case A of
        {error, Error} ->
            Error;
...
-spec be_some(gleam@option:option(FIQ)) -> FIQ.
be_some(A) ->
    case A of
        {some, Value} ->
            Value;
